...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener('click', function(e) {
    // Skip animation inside overlay, cart, auth modal, or mobile drawer
    if (e.target.closest('.windows-overlay, .cart-sidebar, .cart-overlay, .modal-overlay, .mobile-drawer, .drawer-overlay, .admin-overlay, .detail-section')) return;
    ...
    // Defer navigation links until animation completes
    var deferNav = null;
    var targetEl = e.target.closest('a');
    if (targetEl && targetEl.href && !targetEl.hasAttribute('onclick')) {
      var navTarget = targetEl.href;
      // Don't defer javascript: or # links
      if (navTarget && navTarget.indexOf('javascript:') !== 0 && navTarget !== '#') {
        e.preventDefault();
    ...
    // Pick origin direction randomly
    let sx, sy;
    switch (Math.floor(Math.random() * 4)) {
      case 0: sx = -10; sy = 20 + Math.random() * (vh - 40); break;
      case 1: sx = vw + 10; sy = 20 + Math.random() * (vh - 40); break;
      case 2: sx = 20 + Math.random() * (vw - 40); sy = -10; break;
      case 3: sx = 20 + Math.random() * (vw - 40); sy = vh + 10; break;
    }

    const socket = document.createElement('div');
    socket.className = 'click-socket';
    socket.style.left = cx + 'px';
    socket.style.top = cy + 'px';
    socket.innerHTML = '<div class="socket-face"><span class="socket-slot"></span><span class="socket-slot"></span></div>';
    document.body.appendChild(socket);

    // Plug head at cable end
    const plug = document.createElement('div');
    plug.className = 'plug-head';
    plug.style.left = cx + 'px';
    plug.style.top = cy + 'px';
    plug.innerHTML = '<div class="plug-prong"></div><div class="plug-prong"></div>';
    document.body.appendChild(plug);

    // Single continuous SVG cable with twist (cubic bezier S-curve)
    ...
    const amp = totalDist * (0.3 + Math.random() * 0.3);
    const t1x = sx + dx * 0.25 + nx * amp * (0.7 + Math.random() * 0.6);
    const t1y = sy + dy * 0.25 + ny * amp * (0.7 + Math.random() * 0.6);
    const t2x = sx + dx * 0.75 + nx * amp * (-0.7 - Math.random() * 0.6);
    const t2y = sy + dy * 0.75 + ny * amp * (-0.7 - Math.random() * 0.6);
    ...
    const d = `M ${sx} ${sy} C ${t1x} ${t1y} ${t2x} ${t2y} ${cx} ${cy}`;
    ...
    shadow.style.filter = 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))';
    const pathLen = shadow.getTotalLength();
    shadow.setAttribute('stroke-dasharray', pathLen);
    shadow.setAttribute('stroke-dashoffset', pathLen);
    svg.appendChild(shadow);

    // Main cable body (thick black)
    ...
    // Highlight layer for 3D rounded look
    const hl = document.createElementNS(svgNS, 'path');
    hl.setAttribute('d', d);
    hl.setAttribute('fill', 'none');
    hl.setAttribute('stroke', '#3a3a3a');
    hl.setAttribute('stroke-width', '2.5');
    hl.setAttribute('stroke-linecap', 'round');
    hl.setAttribute('stroke-dasharray', pathLen);
    hl.setAttribute('stroke-dashoffset', pathLen);
    svg.appendChild(hl);
    ...
    edge.setAttribute('stroke-dasharray', pathLen);
    edge.setAttribute('stroke-dashoffset', pathLen);
    edge.style.opacity = '0.3';
    svg.appendChild(edge);

    // Single energy bolt traveling from origin to plug
    const energy = document.createElementNS(svgNS, 'path');
    energy.setAttribute('d', d);
    energy.setAttribute('fill', 'none');
    energy.setAttribute('stroke', '#4fc3f7');
    energy.setAttribute('stroke-linecap', 'round');
    energy.setAttribute('stroke-width', '4');
    energy.style.opacity = '0';
    energy.style.filter = 'drop-shadow(0 0 10px rgba(79,195,247,0.9)) drop-shadow(0 0 25px rgba(79,195,247,0.5))';
    ...
    requestAnimationFrame(() => {
      [shadow, path, hl, edge].forEach((el, i) => {
        el.style.transition = `stroke-dashoffset 0.6s cubic-bezier(0.22, 1, 0.36, 1) ${i * 0.03}s`;
        el.style.strokeDashoffset = '0';
      });
      socket.classList.add('active');
      plug.classList.add('active');

      // Single bolt travels origin → plug, then loops
      const boltLen = 60;
      const gap = pathLen - boltLen;
      energy.setAttribute('stroke-dasharray', `${boltLen} ${gap}`);
      core.setAttribute('stroke-dasharray', `${boltLen} ${gap}`);
      const dur = Math.max(500, pathLen * 0.5);
      [energy, core].forEach(el => {
        el.style.opacity = '1';
        el.animate([
          { strokeDashoffset: pathLen, opacity: 0, offset: 0 },
          { strokeDashoffset: pathLen * 0.9, opacity: 1, offset: 0.02 },
          { strokeDashoffset: 0, opacity: 1, offset: 0.85 },
          { strokeDashoffset: -boltLen, opacity: 0, offset: 0.98 },
          { strokeDashoffset: -boltLen, opacity: 0, offset: 1 }
        ], { duration: dur, iterations: Infinity, easing: 'linear' });
      });

      // Short circuit sparks at plug point
      function phuljhadiBurst() {
        const variant = Math.random();
        const sparkCount = 40 + Math.floor(Math.random() * 60);
        const spreadRadius = 60 + Math.random() * 80;
        const gravityPull = 6 + Math.random() * 16;
        const durationFactor = 0.6 + Math.random() * 0.8;
        // Center glow
        const glow = document.createElementNS(svgNS, 'circle');
        glow.setAttribute('cx', cx);
        glow.setAttribute('cy', cy);
        const glowHue = 185 + Math.random() * 35;
        glow.setAttribute('fill', `rgba(${50 + Math.random()*100}, ${150 + Math.random()*80}, 255, ${0.3 + Math.random()*0.4})`);
        glow.style.filter = `drop-shadow(0 0 ${50 + Math.random()*60}px rgba(79,195,247,${0.5 + Math.random()*0.4})) drop-shadow(0 0 ${100 + Math.random()*100}px rgba(79,195,247,${0.2 + Math.random()*0.3}))`;
        svg.appendChild(glow);
        glow.animate([
          { r: 5 + Math.random() * 8, opacity: 1, offset: 0 },
          { r: 20 + Math.random() * 25, opacity: 0.3 + Math.random() * 0.3, offset: 0.2 },
          { r: 40 + Math.random() * 50, opacity: 0, offset: 0.7 }
        ], { duration: 400 + Math.random() * 400, easing: 'ease-out' }).onfinish = () => glow.remove();
        // White hot core
        const core = document.createElementNS(svgNS, 'circle');
        core.setAttribute('cx', cx);
        core.setAttribute('cy', cy);
        core.setAttribute('fill', '#fff');
        core.style.filter = `drop-shadow(0 0 ${10 + Math.random()*15}px #4fc3f7)`;
        svg.appendChild(core);
        core.animate([
          { r: 2 + Math.random() * 3, opacity: 1, offset: 0 },
          { r: 8 + Math.random() * 10, opacity: 0, offset: 0.4 + Math.random() * 0.3 }
        ], { duration: 300 + Math.random() * 300 }).onfinish = () => core.remove();
        // Spark streams radiating outward in all directions
        for (let i = 0; i < sparkCount; i++) {
          const angle = Math.random() * Math.PI * 2;
          const dist = spreadRadius * (0.3 + Math.random() * 0.7);
          const segs = 2 + Math.floor(Math.random() * 6);
          const pts = [{ x: cx, y: cy }];
          let x = cx, y = cy;
          const dirDrift = (Math.random() - 0.5) * 10;
          for (let j = 0; j < segs; j++) {
            const t = (j + 1) / segs;
            const gravity = t * t * gravityPull * (0.5 + Math.random() * 0.5);
            const drift = (Math.random() - 0.5) * 8;
            x += Math.cos(angle) * (dist / segs) + drift;
            y += Math.sin(angle) * (dist / segs) + gravity + drift * 0.3;
            pts.push({ x, y });
          }
          ...
          spark.setAttribute('stroke', `hsl(${hue}, 100%, ${light}%)`);
          spark.setAttribute('stroke-width', 0.3 + Math.random() * 1.5);
          spark.setAttribute('stroke-linecap', 'round');
          svg.appendChild(spark);
          spark.animate([
            { opacity: 1, offset: 0 },
            { opacity: 0.7 + Math.random() * 0.3, offset: 0.1 },
            { opacity: 0, offset: 1 }
          ], { duration: (150 + Math.random() * 400) * durationFactor, easing: 'ease-out' }).onfinish = () => spark.remove();
        }
        // Tiny bright dots spraying everywhere (crackle effect)
        const dotCount = 10 + Math.floor(Math.random() * 25);
        for (let i = 0; i < dotCount; i++) {
          const angle = Math.random() * Math.PI * 2;
          const dist = spreadRadius * (0.1 + Math.random() * 0.8);
          const dot = document.createElementNS(svgNS, 'circle');
          dot.setAttribute('cx', cx + Math.cos(angle) * dist + (Math.random() - 0.5) * 10);
          dot.setAttribute('cy', cy + Math.sin(angle) * dist + (dist / 10) ** 2 * (0.5 + Math.random() * 0.5));
          dot.setAttribute('r', 0.3 + Math.random() * 2);
          dot.setAttribute('fill', Math.random() > 0.5 ? '#fff' : `hsl(${185 + Math.random()*35}, 100%, ${70 + Math.random()*30}%)`);
          dot.style.filter = `drop-shadow(0 0 ${2 + Math.random()*4}px rgba(79,195,247,${0.5 + Math.random()*0.5}))`;
          svg.appendChild(dot);
          dot.animate([
            { opacity: 1, offset: 0 },
            { opacity: 0, offset: 1 }
          ], { duration: (80 + Math.random() * 250) * durationFactor, easing: 'ease-out' }).onfinish = () => dot.remove();
        }
        // Occasional bright flash pop (crackle)
        const popCount = Math.random() > 0.4 ? 1 + Math.floor(Math.random() * 3) : 0;
        for (let p = 0; p < popCount; p++) {
          const pop = document.createElementNS(svgNS, 'circle');
          pop.setAttribute('cx', cx + (Math.random() - 0.5) * 30);
          pop.setAttribute('cy', cy + (Math.random() - 0.5) * 30);
          pop.setAttribute('fill', `rgba(79,195,247,${0.5 + Math.random()*0.5})`);
          pop.style.filter = `drop-shadow(0 0 ${10 + Math.random()*20}px rgba(79,195,247,${0.5 + Math.random()*0.5}))`;
          svg.appendChild(pop);
          pop.animate([
            { r: 1 + Math.random() * 2, opacity: 1, offset: 0 },
            { r: 8 + Math.random() * 15, opacity: 0, offset: 1 }
          ], { duration: 50 + Math.random() * 100 }).onfinish = () => pop.remove();
        }
        // Branching secondary sparks (like sparkler bits flying off)
        if (Math.random() > 0.5) {
          const branchCount = 4 + Math.floor(Math.random() * 8);
          for (let i = 0; i < branchCount; i++) {
            const angle = Math.random() * Math.PI * 2;
            const dist = 20 + Math.random() * 60;
            const bx = cx + Math.cos(angle) * dist;
            const by = cy + Math.sin(angle) * dist + (dist / 12) ** 2;
            for (let j = 0; j < 3 + Math.floor(Math.random() * 4); j++) {
              const ba = Math.random() * Math.PI * 2;
              const bd = 3 + Math.random() * 15;
              const seg = document.createElementNS(svgNS, 'path');
              const ex = bx + Math.cos(ba) * bd;
              const ey = by + Math.sin(ba) * bd + bd * 0.1;
              seg.setAttribute('d', `M ${bx} ${by} L ${ex} ${ey}`);
              seg.setAttribute('fill', 'none');
              seg.setAttribute('stroke', `hsl(${185 + Math.random()*35}, 100%, ${70 + Math.random()*30}%)`);
              seg.setAttribute('stroke-width', 0.3 + Math.random() * 0.8);
              seg.setAttribute('stroke-linecap', 'round');
              svg.appendChild(seg);
              seg.animate([{ opacity: 1, offset: 0 },{ opacity: 0, offset: 1 }], { duration: (60 + Math.random() * 120) * durationFactor }).onfinish = () => seg.remove();
            }
          }
        }
      }

      // Trigger sparks periodically, synced with bolt arrival
      const sparkInterval = setInterval(phuljhadiBurst, dur * 0.88);
      ...
      setTimeout(() => {
        ...
        plug.remove();
        svg.remove();
        if (deferNav) window.location.href = deferNav;
      }, 2400);
    });
  }, { capture: true });
